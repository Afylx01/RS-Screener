...
class AdvancedScanner:
    ...
    def calculate_indicators(self, symbol: str, data: pd.DataFrame, rs_value: Optional[float]) -> Optional[Dict]:
        """
        Calculates all required technical indicators for a single stock using pandas_ta.
        """
        if data.empty or len(data) < 250:  # Ensure enough data for EMA 200
            log.warning(f"Skipping {symbol} due to insufficient data for indicator calculation.")
            return None
    ...
    def check_entry_signals(self, indicators: Dict, data: pd.DataFrame) -> Dict:
    ...
    def run_advanced_scan(self, market_data: Dict[str, pd.DataFrame], rs_lookup: Dict[str, float]):
        results = []
        log.info("Starting advanced HHHL + ADX analysis...")
        for symbol, data in tqdm(market_data.items(), desc="Running Advanced Analysis"):
            hhhl_result = self.hhhl_scanner.scan_single_stock(symbol, df_data=data)
            if not hhhl_result: continue

            rs_value = rs_lookup.get(symbol.replace(".NS", ""))
            indicators = self.calculate_indicators(symbol, data, rs_value)
            ...
            if indicators['tradeable']:
                signals = self.check_entry_signals(indicators, data)
