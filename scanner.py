...
def run_scan(all_stock_data, market_caps, scan_date):
    ...
    for symbol, data in tqdm(all_stock_data.items(), desc=f"Scanning for {scan_date.strftime('%Y-%m-%d')}"):
        ...
        if '52w_high' not in day_data or day_data['close'] < SCAN_CONFIG['min_52_week_ratio'] * day_data['52w_high']:
            continue

        # Condition 12: RS55 Condition
        rs_mode = SCAN_CONFIG['rs_scan_mode']
        if rs_mode == 'positive' and day_data.get('RS55_Today', 0) <= 0:
            continue
        elif rs_mode == 'crossover' and not (day_data.get('RS55_Today', 0) > 0 and day_data.get('RS55_Yesterday', 0) <= 0):
            continue
        ...
        results.append({
            ...
            'RS55_Today': day_data.get('RS55_Today', 0),
            'RS55_Yesterday': day_data.get('RS55_Yesterday', 0),
            '52W_High_Ratio': day_data['close'] / day_data['52w_high'] if '52w_high' in day_data else 0,
            'Golden_Cross': True,
        })
